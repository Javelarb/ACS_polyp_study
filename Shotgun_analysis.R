...
#samp_type_perma  
perma1 <- adonis(formula = merged_OTU_table[,16:ncol(merged_OTU_table)] ~ as.numeric(merged_OTU_table$BMI) + 
...
#asp perma
perma2 <- adonis(formula = asp_OTU[,16:ncol(asp_OTU)] ~ as.numeric(asp_OTU$BMI) + as.numeric(asp_OTU$Age) + asp_OTU$Ethnicity 
...
top.coef2$Taxa <- row.names(top.coef2)
...
#fecal perma
perma3 <- adonis(formula = fecal_OTU[,16:ncol(fecal_OTU)] ~ fecal_OTU$MostMalignantPolypType 
...
#Lavage
perma4 <- adonis(formula = lavage_OTU[,16:ncol(lavage_OTU)] ~ lavage_OTU$MostMalignantPolypType + lavage_OTU$PrepType + 
...
training_healthy = asp_OTU %>% select(MostMalignantPolypType, !filter_list$Var2) %>% 
  select(!Row.names:InFinalAnalysis) %>% filter(MostMalignantPolypType == "healthy") %>% sample_frac(2/3)
...
test_healthy = asp_OTU %>% select(MostMalignantPolypType, !filter_list$Var2) %>% 
  select(!Row.names:InFinalAnalysis) %>% filter(MostMalignantPolypType == "healthy")
...
training_TA = asp_OTU %>% select(MostMalignantPolypType, !filter_list$Var2) %>% 
  select(!Row.names:InFinalAnalysis) %>% filter(MostMalignantPolypType == "TA") %>% sample_frac(2/3)
...
test_TA = asp_OTU %>% select(MostMalignantPolypType, !filter_list$Var2) %>% 
  select(!Row.names:InFinalAnalysis) %>% filter(MostMalignantPolypType == "TA")
...
training_serr = asp_OTU %>% select(MostMalignantPolypType, !filter_list$Var2) %>% 
  select(!Row.names:InFinalAnalysis) %>% filter(MostMalignantPolypType == "serrated") %>% sample_frac(2/3)
...
test_serr = asp_OTU %>% select(MostMalignantPolypType, !filter_list$Var2) %>% 
  select(!Row.names:InFinalAnalysis) %>% filter(MostMalignantPolypType == "serrated")
